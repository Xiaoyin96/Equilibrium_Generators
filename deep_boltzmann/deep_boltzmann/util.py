...
def _linlogcut_tf_constantclip(x, a=0, b=1000):
    # cutoff x after b - this should also cutoff infinities
    x = tf.where(x < b, x, x / tf.maximum(1., tf.stop_gradient(x) / b))
    # cutoff x after b - this should also cutoff infinities
    #x = tf.where(x < b, x, b * tf.ones(tf.shape(x)))
    # log after a
    y = a + tf.where(x < a, x - a, tf.math.log(x - a + 1))
...
def _clip_high_tf(x, cutoff_min):
    return x / tf.maximum(1., tf.stop_gradient(x) / cutoff_min)

def transform_gradient(op, transform_fn):
    @tf.custom_gradient
    def _call(*args, **kwargs):
        xs = args + tuple(*kwargs.values())
        ys = op(*args, **kwargs)
        if len(xs) > 0:
            def _grad_fun(*grad_ys):
                return transform_fn(xs, ys, tf.gradients(ys, xs, grad_ys))
            return ys, _grad_fun
        else:
            return ys
    return _call

def gradient_clip(op, cutoff_min, cutoff_max):
    def _grad_clip_fn(_, __, grads):
        grads_clipped = []
        for g in grads:
            norm = tf.norm(g, axis=-1, keepdims=True)
            grads_clipped.append(g / norm * linlogcut(norm, a=cutoff_min, b=cutoff_max))
        return grads_clipped
    return transform_gradient(op, _grad_clip_fn)
